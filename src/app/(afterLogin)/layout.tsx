const AfterLoginLayout = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className='flex items-stretch bg-white'>
      {/* leftSection */}
      <header className='flex items-end flex-col flex-grow'>
        <section className='w-[275px h-dvh'>
          <div className='fixed w-[275px] h-dvh flex flex-col'></div>
        </section>
      </header>

      {/* rightSection */}
      <div className='flex items-start h-dvh flex-col flex-grow'>
        <div className='h-full w-[990px] flex justify-between'>
          <main className='w-[600px] h-[200dvh]'>{children}</main>
          <section className='w-[350px] h-full'>
            <form className='fixed h-[42px] w-[350px] rounded-3xl bg-[#eff3f4] mt-1.5 mb-3 flex items-center'>
              <svg
                width={20}
                viewBox='0 0 24 24'
                aria-hidden='true'
                className='ml-5 fill-[#536471]'
              >
                <g>
                  <path d='M10.25 3.75c-3.59 0-6.5 2.91-6.5 6.5s2.91 6.5 6.5 6.5c1.795 0 3.419-.726 4.596-1.904 1.178-1.177 1.904-2.801 1.904-4.596 0-3.59-2.91-6.5-6.5-6.5zm-8.5 6.5c0-4.694 3.806-8.5 8.5-8.5s8.5 3.806 8.5 8.5c0 1.986-.682 3.815-1.824 5.262l4.781 4.781-1.414 1.414-4.781-4.781c-1.447 1.142-3.276 1.824-5.262 1.824-4.694 0-8.5-3.806-8.5-8.5z'></path>
                </g>
              </svg>
              <input
                type='search'
                className='outline-none bg-transparent border-none p-3 ml-[5px] text-sm'
              />
            </form>
          </section>
        </div>
      </div>
      {children}
    </div>
  );
};

export default AfterLoginLayout;
